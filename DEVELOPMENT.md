...
Webots 的驱动轮接触使用单值（各向同性）库仑摩擦：normal 为 0.90，
low/patch 为 0.25。不能把第二个较低系数当作“滚动阻力”写进
`coulombFriction`；两值形式会把摩擦绑定到地面纹理 U/V 方向，使同一车辆
沿地图 X/Y 轴获得不同牵引力，表现为编码器已经转够但车身实际欠程。
直行动作的可用扭矩默认限制为电机峰值的 30%，足以克服起步静摩擦；
转向仍可使用完整峰值。低摩擦、左右不对称和局部低摩擦差异由独立的
`contactMaterial` 系数表达，不依赖地图方向。
物理闭环还把直线轮速限制在峰值的 10% 牵引包线内；上层请求值和实际应用值
...
- 每次 run 独立冻结 `raw_metrics.json`，原始指标不可被后续评分规则覆盖。
- 默认 `score-profile-v1` 按完成与终点 35、地图准确率 20、定位与方向 15、路径与时间 12、动作精度 8、安全与稳定 10 计分。
- `events.jsonl`、`replay.json`、分项成绩和视频统一归档；视频文件保存 SHA-256。
- 回放以事件的 `monotonic_ns` 为基准，统一同步轨迹、仿真真值、遥测、动作、参数、审批和安全事件。
- Dashboard 可从历史运行列表重新选择 run，点击关键事件刻度跳转；视频缺失时继续提供结构化回放。
- 仿真录像支持 Xvfb/Webots `x11grab`；实车录像接收 640×360、5–10 FPS JPEG，服务端按 3 Mbps 限制并由 ffmpeg 封装 MP4。
- 普通视频保留 30 天，JSONL、原始指标和回放清单保留 180 天；固定、最佳、基准、事故和比赛证据不自动删除。

约束：

- 缺失证据必须显示为缺失并按 0 分处理，不生成伪指标。
- 仿真真值只进入评分和独立回放轨，不进入定位融合输入。
- 视频启动、写入或封装失败只把 run 标记为 `media_incomplete`，不得把已完成任务改为失败。
- 清理器只删除数据目录内的普通文件；固定或被报告引用的证据必须保留。
- Task 8 只提供录像器和证据注册接口；RDK 相机上传通道在实车 Agent 任务接线，服务器 Xvfb 显示号在部署任务接线。

验收：

- 同一份原始指标可用新 score profile 重算，原文件字节不变。
- 任务结束自动生成原始指标、综合分、JSONL 索引和同步回放清单。
